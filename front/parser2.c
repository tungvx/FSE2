...
static AST classhead() {
	...
	a1 = name();
	if (a1) {
	    insert_SYM(get_text(a1), 0, tGLOBAL, 0);
	    set_nodetype(a1, tCLASS);
	}
...
static AST primtype() {
	...
	default:      parse_error("expected primtype, struct or class as type"); break;
    }
    gettoken();
    return a;
}
...
static AST stmt() {
	...
	case ID: /* ASN or CALL */
		...
		if (nodetype(type) == tCLASS){
		    ...
		    set_sons(a1,function_name, args, n, 0);
		    if (t->sym == ';') gettoken();
		} else {
...
static AST func_name_ref(AST class){
	...
	char *s = strdup(t->text);
	gettoken();
	class = get_father(get_father(class));
	AST funcdecls, funcdecl;
	get_sons(class, 0, 0, &funcdecls, 0);
	do {
	    get_sons(funcdecls, &funcdecl, &funcdecls, 0, 0);
	    if (funcdecl){
		AST funcname = 0;
		get_sons(funcdecl, &funcname, 0, 0, 0);
		if (strcmp(get_text(funcname), s) == 0) return funcname;
	    } else break;
	} while (true);
    } else parse_error("expected ID");
    parse_error("Function not found in the class");
    return a;
}
